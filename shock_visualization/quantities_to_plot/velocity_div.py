...
from shock_visualization.quantities import (
    Density, Field, Velocity, DivergenceOfField, PlotParams
)
...
divVi = DivergenceOfField(abs(V0), 1.0, 0.0, Vix, Viy, Viz, norm=True)
divVe = DivergenceOfField(abs(V0), 1.0, 0.0, Vex, Vey, Vez, norm=True)
...
curl_plot_params = PlotParams(
    colspan = colspan,
    fig_name = "step_div_ion",
    plot_path = f"{PATH_TO_PLOT}/vel/div",
    plot_name = "{}".format(r"$\nabla \delta v_i/v_0$"),
    limits = lims,
    labels = labels,
    cmap = cmap,
...
divVi.plot_params = deepcopy(curl_plot_params)

curl_plot_params.fig_name = "step_ele"
curl_plot_params.plot_name = "{}".format(r"$\nabla \delta v_e/v_0$")
divVe.plot_params = deepcopy(curl_plot_params)
